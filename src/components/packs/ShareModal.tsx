...
export default function ShareModal({
  ...
  useEffect(() => {
    ...
    if (packId) {
      ...
      // If no packId but we have streams, create a URL with stream parameters
      const streamsParam = streams.map(s => `stream=${encodeURIComponent(s)}`).join('&');
      setShareUrl(`${origin}/viewer?${streamsParam}`);
    } else {
  ...
  const handleShare = (platform: 'twitter' | 'facebook' | 'reddit' | 'discord') => {
    ...
    switch (platform) {
      ...
      case 'discord':
        ...
        const streamsList = streams.length > 0 ? `\nStreams: ${streams.join(', ')}` : '';
        const discordMessage = `**${packTitle}**\n${packDescription}${streamsList}\n${shareUrl}`;
  ...
  const ogImageUrl = packId 
    ? `${typeof window !== 'undefined' ? window.location.origin : ''}/viewer/opengraph-image?pack=${packId}`
    : `${typeof window !== 'undefined' ? window.location.origin : ''}/api/og?title=${encodeURIComponent(packTitle)}&streams=${encodeURIComponent(streams.join(','))}`;

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#18181b] rounded-lg border border-[#2d2d3a] max-w-lg w-full p-6 relative">
        {/* Close button */}
        <button 
          onClick={onClose}
